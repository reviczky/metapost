...
pointer mp_p_over_v (MP mp,pointer p, scaled v, quarterword 
  ...
  mp_link(r)=p; 
  value(p)=mp_make_scaled(mp, value(p),v);
  return mp_link(temp_head);
}
...
static pointer mp_single_dependency (MP mp,pointer p) {
  ...
  if ( m>28 ) {
    ...
    value(q)=(integer)two_to_the(28-m); 
    set_mp_info(q,p);
...
pp=mp_info(s); 
v=value(s);
if ( t==mp_dependent ) value(s)=-fraction_one; 
else value(s)=-unity;
r=dep_list(pp); 
mp_link(s)=r;
...
static void mp_dep_mult (MP mp,pointer p, integer v, boolean v_is_scaled) {
  ...
  if ( p==null ) {
    ...
    if ( v_is_scaled ) 
      value(p)=mp_take_scaled(mp, value(p),v);
    else 
      value(p)=mp_take_fraction(mp, value(p),v);
    return;
  };
  t=mp_type(q); 
  q=dep_list(q); 
...
static pointer mp_sort_in (MP mp,scaled v) {
  ...
  if ( v<value(q) ) {
    r=mp_get_value_node(mp); value(r)=v; mp_link(r)=q; mp_link(p)=r;
  }
  return mp_link(p);
}
