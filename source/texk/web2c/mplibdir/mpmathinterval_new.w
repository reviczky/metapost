...
const char *COMPILED_MPFI_VERSION_STRING = MPFR_VERSION_STRING;
...
@d checkZero(dec)  if (mpfi_is_zero(dec) && mpfi_negative_p(dec)) {
     mpfi_set_d(dec,0.0);
   }
...
int binary_number_check (mpfi_t dec)
{
   ...
   if (!mpfi_number_p(dec)) {
      ...
      if (mpfi_inf_p(dec)) {
        ...
        mpfi_set_zero(dec,1); /* 1 == positive */
      }
   }
   checkZero(dec);
   return test;
...
@d set_cur_mod(A) mpfi_set((mpfi_ptr)(mp->cur_mod_->data.n.data.num),A, ROUNDING)
...
void mp_interval_velocity (MP mp, mp_number *ret, mp_number st, mp_number ct, mp_number sf,
  ...
  mpfi_div (arg1,sf.data.num, i16, ROUNDING); // arg1 = sf / 16
  mpfi_sub (arg1,st.data.num, arg1, ROUNDING); // arg1 = st - arg1
  mpfi_div (arg2,st.data.num, i16, ROUNDING); // arg2 = st / 16
  mpfi_sub (arg2,sf.data.num, arg2, ROUNDING); // arg2 = sf - arg2
  mp_interval_take_fraction (mp, acc, arg1, arg2); // acc = (arg1 * arg2) / fmul

  mpfi_set (arg1, acc, ROUNDING);
  mpfi_sub (arg2, ct.data.num, cf.data.num, ROUNDING); // arg2 = ct - cf
  mp_interval_take_fraction (mp, acc, arg1, arg2); // acc = (arg1 * arg2 ) / fmul

  mpfi_sqrt(arg1, two_mpfi_t, ROUNDING); // arg1 = sqrt(2)
  mpfi_mul(arg1, arg1, fone, ROUNDING);     // arg1 = arg1 * fmul
  mp_interval_take_fraction (mp, r1, acc, arg1);  // r1 = (acc * arg1) / fmul
  mpfi_add(num, ftwo, r1, ROUNDING);             // num = ftwo + r1
  
  mpfi_sub(arg1,sqrtfive, one, ROUNDING);   // arg1 = sqrt(5) - 1
  mpfi_mul(arg1,arg1,fhalf, ROUNDING);      // arg1 = arg1 * fmul/2
  mpfi_mul(arg1,arg1,three_mpfi_t, ROUNDING); // arg1 = arg1 * 3

  mpfi_sub(arg2,three_mpfi_t, sqrtfive, ROUNDING); // arg2 = 3 - sqrt(5)
  mpfi_mul(arg2,arg2,fhalf, ROUNDING);            // arg2 = arg2 * fmul/2
  mpfi_mul(arg2,arg2,three_mpfi_t, ROUNDING);  // arg2 = arg2 * 3
  ...
  mpfi_set_si(denom, fraction_three, ROUNDING);  // denom = 3fmul
  mpfi_add(denom, denom, r1, ROUNDING);     // denom = denom + r1
  mpfi_add(denom, denom, r2, ROUNDING);     // denom = denom + r2

  if (!mpfi_equal_p(t.data.num, one)) {                 // t != 1
    mpfi_div(num, num, t.data.num, ROUNDING); // num = num / t
  }
  mpfi_set(r2, num, ROUNDING);                        // r2 = num / 4
  mpfi_div(r2, r2, four_mpfi_t, ROUNDING);
  if (mpfi_less_p(denom,r2)) { // num/4 >= denom => denom < num/4
...
@d no_crossing   { mpfi_set(ret->data.num, fraction_one_plus_mpfi_t, ROUNDING); goto RETURN; }
@d one_crossing  { mpfi_set(ret->data.num, fraction_one_mpfi_t, ROUNDING); goto RETURN; }
@d zero_crossing { mpfi_set(ret->data.num, zero, ROUNDING); goto RETURN; }
...
void mp_number_floor (mp_number *i) {
  mpfi_rint_floor(i->data.num, i->data.num, MPFI_RNDD);
}
...
static void mp_interval_m_unif_rand (MP mp, mp_number *ret, mp_number x_orig) {
  ...
  r = mpfi_get_str(NULL,    /* |char *str|,         */     
                  &e,       /* |mpfi_exp_t *expptr|,*/
                  10,       /* |int b|,             */
                  0,        /* |size_t n|,          */
                  ret->data.num, /* |mpfi_t op|,    */      
