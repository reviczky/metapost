...
@c
int mp_save_png_to_file (MP mp, const bitmap_t * bitmap, const char *path)
{
    FILE * fp;
    ...
    /* |3| appears the best tradeoff between disk size and speed */
    png_set_compression_level(png_ptr, 3); 

    /* setup some information (*/
    if (1) {
        png_text  text[6];
	char *a, *b, *c, *d; /* to get rid of a typecase warning */
        int  num_text = 0;
  
        text[num_text].compression = PNG_TEXT_COMPRESSION_NONE;
        a = xstrdup("Title"); text[num_text].key = a;
        b = xstrdup(path);    text[num_text].text = b;
        ++num_text;
        text[num_text].compression = PNG_TEXT_COMPRESSION_NONE;
        c = xstrdup("Author"); text[num_text].key = c;
	d = xstrdup("Generated by Metapost version " metapost_version);
        text[num_text].text = d;
        ++num_text;
...
int mp_png_gr_ship_out (mp_edge_object *hh, int qprologues, int standalone) {
  ...
  }
  {
    ...
    mp_save_png_to_file (mp, &bitmap, ss);
    cairo_surface_destroy (mp->png->surface);
#else
    char *ss = mp_get_output_file_name (mp);
    cairo_destroy (mp->png->cr);
    cairo_surface_write_to_png(mp->png->surface, ss);
    cairo_surface_destroy (mp->png->surface);
#endif
  }
  return 1;
