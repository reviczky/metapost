...
#define mpfi_negative_p(a) (mpfi_sgn((a) ) <0) 
#define mpfi_positive_p(a) (mpfi_sgn((a) ) > 0) 
#define checkZero(dec) if(mpfi_zero_p(dec) &&mpfi_negative_p(dec) ) { \
mpfi_set_zero(dec,1) ; \
} \
...
#define set_cur_mod(A) mpfi_set((mpfi_ptr) (mp->cur_mod_->data.n.data.num) ,A,ROUNDING)  \
...
#define no_crossing {mpfi_set(ret->data.num,fraction_one_plus_mpfi_t,ROUNDING) ;goto RETURN;}
#define one_crossing {mpfi_set(ret->data.num,fraction_one_mpfi_t,ROUNDING) ;goto RETURN;}
#define zero_crossing {mpfi_set(ret->data.num,zero,ROUNDING) ;goto RETURN;} \


#line 30 "mpmathinterval_new.w"


/*:1*//*2:*/
#line 32 "mpmathinterval_new.w"

/*5:*/
#line 71 "mpmathinterval_new.w"
...
static int binary_number_check(mpfi_t dec);
static char*mp_binnumber_tostring(mpfi_t n);
static void init_binary_constants(void);
static void free_binary_constants(void);
static mpfi_prec_t precision_digits_to_bits(double i);
static double precision_bits_to_digits(mpfi_prec_t i);

/*:5*//*9:*/
#line 224 "mpmathinterval_new.w"
...
/*:9*//*25:*/
#line 840 "mpmathinterval_new.w"

void mp_interval_make_fraction(MP mp,mpfi_t ret,mpfi_t p,mpfi_t q);

/*:25*//*27:*/
#line 862 "mpmathinterval_new.w"

void mp_interval_take_fraction(MP mp,mpfi_t ret,mpfi_t p,mpfi_t q);

/*:27*//*31:*/
#line 903 "mpmathinterval_new.w"

static void mp_wrapup_numeric_token(MP mp,unsigned char*start,unsigned char*stop);

/*:31*/
#line 33 "mpmathinterval_new.w"
;

/*:2*//*6:*/
#line 156 "mpmathinterval_new.w"

int binary_number_check(mpfi_t dec)
{
int test= false;
if(!mpfi_number_p(dec)){
test= true;
if(mpfi_inf_p(dec)){
mpfi_set(dec,EL_GORDO_mpfi_t,ROUNDING);
if(mpfi_negative_p(dec)){
mpfi_neg(dec,dec,ROUNDING);
}
}else{
mpfi_set_zero(dec,1);
}
...
/*:6*//*7:*/
#line 184 "mpmathinterval_new.w"

static double precision_bits;
mpfi_prec_t precision_digits_to_bits(double i)
{
return i/log10(2);
}
double precision_bits_to_digits(mpfi_prec_t d)
{
return d*log10(2);
}
...
mpfi_set_si(one,1,ROUNDING);
mpfi_set_si(minusone,-1,ROUNDING);
mpfi_set_si(zero,0,ROUNDING);
mpfi_set_si(two_mpfi_t,two,ROUNDING);
mpfi_set_si(three_mpfi_t,three,ROUNDING);
mpfi_set_si(four_mpfi_t,four,ROUNDING);
mpfi_set_si(fraction_multiplier_mpfi_t,fraction_multiplier,ROUNDING);
mpfi_set_si(fraction_one_mpfi_t,fraction_one,ROUNDING);
mpfi_set_si(fraction_one_plus_mpfi_t,(fraction_one+1),ROUNDING);
mpfi_set_si(angle_multiplier_mpfi_t,angle_multiplier,ROUNDING);
mpfi_set_str(PI_mpfi_t,PI_STRING,10,ROUNDING);
mpfi_set_d(epsilon_mpfi_t,epsilon,ROUNDING);
mpfi_set_str(EL_GORDO_mpfi_t,EL_GORDO,10,ROUNDING);
initialized= true;
}
}
...
mpfi_set_d(math->precision_default.data.num,DEF_PRECISION,ROUNDING);
mp_new_number(mp,&math->precision_max,mp_scaled_type);
mpfi_set_d(math->precision_max.data.num,MAX_PRECISION,ROUNDING);
mp_new_number(mp,&math->precision_min,mp_scaled_type);

mpfi_set_d(math->precision_min.data.num,2.0,ROUNDING);

mp_new_number(mp,&math->epsilon_t,mp_scaled_type);
mpfi_set(math->epsilon_t.data.num,epsilon_mpfi_t,ROUNDING);
mp_new_number(mp,&math->inf_t,mp_scaled_type);
mpfi_set(math->inf_t.data.num,EL_GORDO_mpfi_t,ROUNDING);
mp_new_number(mp,&math->warning_limit_t,mp_scaled_type);
mpfi_set_d(math->warning_limit_t.data.num,warning_limit,ROUNDING);
mp_new_number(mp,&math->one_third_inf_t,mp_scaled_type);
mpfi_div(math->one_third_inf_t.data.num,math->inf_t.data.num,three_mpfi_t,ROUNDING);
mp_new_number(mp,&math->unity_t,mp_scaled_type);
mpfi_set(math->unity_t.data.num,one,ROUNDING);
mp_new_number(mp,&math->two_t,mp_scaled_type);
mpfi_set_si(math->two_t.data.num,two,ROUNDING);
mp_new_number(mp,&math->three_t,mp_scaled_type);
mpfi_set_si(math->three_t.data.num,three,ROUNDING);
mp_new_number(mp,&math->half_unit_t,mp_scaled_type);
mpfi_set_d(math->half_unit_t.data.num,half_unit,ROUNDING);
mp_new_number(mp,&math->three_quarter_unit_t,mp_scaled_type);
mpfi_set_d(math->three_quarter_unit_t.data.num,three_quarter_unit,ROUNDING);
...
{
mpfi_div_si(math->arc_tol_k.data.num,one,4096,ROUNDING);

}
mp_new_number(mp,&math->fraction_one_t,mp_fraction_type);
mpfi_set_si(math->fraction_one_t.data.num,fraction_one,ROUNDING);
mp_new_number(mp,&math->fraction_half_t,mp_fraction_type);
mpfi_set_si(math->fraction_half_t.data.num,fraction_half,ROUNDING);
mp_new_number(mp,&math->fraction_three_t,mp_fraction_type);
mpfi_set_si(math->fraction_three_t.data.num,fraction_three,ROUNDING);
mp_new_number(mp,&math->fraction_four_t,mp_fraction_type);
mpfi_set_si(math->fraction_four_t.data.num,fraction_four,ROUNDING);

mp_new_number(mp,&math->three_sixty_deg_t,mp_angle_type);
mpfi_set_si(math->three_sixty_deg_t.data.num,360*angle_multiplier,ROUNDING);
mp_new_number(mp,&math->one_eighty_deg_t,mp_angle_type);
mpfi_set_si(math->one_eighty_deg_t.data.num,180*angle_multiplier,ROUNDING);

mp_new_number(mp,&math->one_k,mp_scaled_type);
mpfi_set_d(math->one_k.data.num,1.0/64,ROUNDING);
mp_new_number(mp,&math->sqrt_8_e_k,mp_scaled_type);
{
mpfi_set_d(math->sqrt_8_e_k.data.num,112428.82793/65536.0,ROUNDING);

}
mp_new_number(mp,&math->twelve_ln_2_k,mp_fraction_type);
{
mpfi_set_d(math->twelve_ln_2_k.data.num,139548959.6165/65536.0,ROUNDING);

}
mp_new_number(mp,&math->coef_bound_k,mp_fraction_type);
mpfi_set_d(math->coef_bound_k.data.num,coef_bound,ROUNDING);
mp_new_number(mp,&math->coef_bound_minus_1,mp_fraction_type);
mpfi_set_d(math->coef_bound_minus_1.data.num,coef_bound-1/65536.0,ROUNDING);
mp_new_number(mp,&math->twelvebits_3,mp_scaled_type);
{
mpfi_set_d(math->twelvebits_3.data.num,1365/65536.0,ROUNDING);

}
mp_new_number(mp,&math->twentysixbits_sqrt2_t,mp_fraction_type);
{
mpfi_set_d(math->twentysixbits_sqrt2_t.data.num,94906265.62/65536.0,ROUNDING);

}
mp_new_number(mp,&math->twentyeightbits_d_t,mp_fraction_type);
{
mpfi_set_d(math->twentyeightbits_d_t.data.num,35596754.69/65536.0,ROUNDING);

}
mp_new_number(mp,&math->twentysevenbits_sqrt2_d_t,mp_fraction_type);
{
mpfi_set_d(math->twentysevenbits_sqrt2_d_t.data.num,25170706.63/65536.0,ROUNDING);

}

mp_new_number(mp,&math->fraction_threshold_t,mp_fraction_type);
mpfi_set_d(math->fraction_threshold_t.data.num,fraction_threshold,ROUNDING);
mp_new_number(mp,&math->half_fraction_threshold_t,mp_fraction_type);
mpfi_set_d(math->half_fraction_threshold_t.data.num,half_fraction_threshold,ROUNDING);
mp_new_number(mp,&math->scaled_threshold_t,mp_scaled_type);
mpfi_set_d(math->scaled_threshold_t.data.num,scaled_threshold,ROUNDING);
mp_new_number(mp,&math->half_scaled_threshold_t,mp_scaled_type);
mpfi_set_d(math->half_scaled_threshold_t.data.num,half_scaled_threshold,ROUNDING);
mp_new_number(mp,&math->near_zero_angle_t,mp_angle_type);
mpfi_set_d(math->near_zero_angle_t.data.num,near_zero_angle,ROUNDING);
mp_new_number(mp,&math->p_over_v_threshold_t,mp_fraction_type);
mpfi_set_d(math->p_over_v_threshold_t.data.num,p_over_v_threshold,ROUNDING);
mp_new_number(mp,&math->equation_threshold_t,mp_scaled_type);
mpfi_set_d(math->equation_threshold_t.data.num,equation_threshold,ROUNDING);
mp_new_number(mp,&math->tfm_warn_threshold_t,mp_scaled_type);
mpfi_set_d(math->tfm_warn_threshold_t.data.num,tfm_warn_threshold,ROUNDING);
...
double d= mpfi_get_d(internal_value(mp_number_precision).data.num,ROUNDING);
...
mpfi_set_zero((mpfi_ptr)(n->data.num),1);
n->type= t;
}

/*:13*//*14:*/
#line 503 "mpmathinterval_new.w"
...
/*:14*//*15:*/
#line 515 "mpmathinterval_new.w"

void mp_set_binary_from_int(mp_number*A,int B){
mpfi_set_si(A->data.num,B,ROUNDING);
}
...
mpfi_sub(c,B.data.num,C.data.num,ROUNDING);
mp_interval_take_fraction(mp,r1,c,t.data.num);
mpfi_sub(A->data.num,B.data.num,r1,ROUNDING);
mpfi_clear(c);
mpfi_clear(r1);
mp_check_mpfi_t(mp,A->data.num);
}
void mp_number_negate(mp_number*A){
mpfi_neg(A->data.num,A->data.num,ROUNDING);
checkZero((mpfi_ptr)A->data.num);
}
void mp_number_add(mp_number*A,mp_number B){
mpfi_add(A->data.num,A->data.num,B.data.num,ROUNDING);
}
void mp_number_substract(mp_number*A,mp_number B){
mpfi_sub(A->data.num,A->data.num,B.data.num,ROUNDING);
}
void mp_number_half(mp_number*A){
mpfi_div_si(A->data.num,A->data.num,2,ROUNDING);
}
void mp_number_halfp(mp_number*A){
mpfi_div_si(A->data.num,A->data.num,2,ROUNDING);
}
void mp_number_double(mp_number*A){
mpfi_mul_si(A->data.num,A->data.num,2,ROUNDING);
}
void mp_number_add_scaled(mp_number*A,int B){
mpfi_add_d(A->data.num,A->data.num,B/65536.0,ROUNDING);
}
void mp_number_multiply_int(mp_number*A,int B){
mpfi_mul_si(A->data.num,A->data.num,B,ROUNDING);
}
void mp_number_divide_int(mp_number*A,int B){
mpfi_div_si(A->data.num,A->data.num,B,ROUNDING);
}
void mp_interval_abs(mp_number*A){
mpfi_abs(A->data.num,A->data.num,ROUNDING);
}
void mp_number_clone(mp_number*A,mp_number B){
mpfi_prec_round(A->data.num,precision_bits,ROUNDING);
mpfi_set(A->data.num,(mpfi_ptr)B.data.num,ROUNDING);
}
...
mpfi_div(A->data.num,A->data.num,fraction_multiplier_mpfi_t,ROUNDING);
}
void mp_number_angle_to_scaled(mp_number*A){
A->type= mp_scaled_type;
mpfi_div(A->data.num,A->data.num,angle_multiplier_mpfi_t,ROUNDING);
}
void mp_number_scaled_to_fraction(mp_number*A){
A->type= mp_fraction_type;
mpfi_mul(A->data.num,A->data.num,fraction_multiplier_mpfi_t,ROUNDING);
}
void mp_number_scaled_to_angle(mp_number*A){
A->type= mp_angle_type;
mpfi_mul(A->data.num,A->data.num,angle_multiplier_mpfi_t,ROUNDING);
}


/*:15*//*17:*/
#line 620 "mpmathinterval_new.w"

int mp_number_to_scaled(mp_number A){
double v= mpfi_get_d(A.data.num,ROUNDING);
return(int)(v*65536.0);
}

/*:17*//*18:*/
#line 630 "mpmathinterval_new.w"

int mp_number_to_int(mp_number A){
int32_t result= 0;
if(mpfi_fits_sint_p(A.data.num,ROUNDING)){
result= mpfi_get_si(A.data.num,ROUNDING);
}
return result;
}
...
double res= 0.0;
if(mpfi_number_p(A.data.num)){
res= mpfi_get_d(A.data.num,ROUNDING);
}
return res;
}
...
return mpfi_equal_p(A.data.num,B.data.num);
}
int mp_number_greater(mp_number A,mp_number B){
return mpfi_greater_p(A.data.num,B.data.num);
}
int mp_number_less(mp_number A,mp_number B){
return mpfi_less_p(A.data.num,B.data.num);
}
int mp_number_nonequalabs(mp_number A,mp_number B){
return!(mpfi_cmpabs(A.data.num,B.data.num)==0);
}

/*:18*//*21:*/
#line 690 "mpmathinterval_new.w"

char*mp_binnumber_tostring(mpfi_t n){
char*str= NULL,*buffer= NULL;
mpfi_exp_t exp= 0;
int neg= 0;
if((str= mpfi_get_str(NULL,&exp,10,0,n,ROUNDING))> 0){
...
/*:22*//*23:*/
#line 787 "mpmathinterval_new.w"

void mp_interval_slow_add(MP mp,mp_number*ret,mp_number A,mp_number B){
mpfi_add(ret->data.num,A.data.num,B.data.num,ROUNDING);
}

/*:23*//*24:*/
#line 830 "mpmathinterval_new.w"

void mp_interval_make_fraction(MP mp,mpfi_t ret,mpfi_t p,mpfi_t q){
mpfi_div(ret,p,q,ROUNDING);
mp_check_mpfi_t(mp,ret);
mpfi_mul(ret,ret,fraction_multiplier_mpfi_t,ROUNDING);
}
...
/*:24*//*26:*/
#line 853 "mpmathinterval_new.w"

void mp_interval_take_fraction(MP mp,mpfi_t ret,mpfi_t p,mpfi_t q){
mpfi_mul(ret,p,q,ROUNDING);
mpfi_div(ret,ret,fraction_multiplier_mpfi_t,ROUNDING);
}
...
/*:26*//*28:*/
#line 875 "mpmathinterval_new.w"

void mp_interval_number_take_scaled(MP mp,mp_number*ret,mp_number p_orig,mp_number q_orig){
mpfi_mul(ret->data.num,p_orig.data.num,q_orig.data.num,ROUNDING);
}


/*:28*//*29:*/
#line 887 "mpmathinterval_new.w"

void mp_interval_number_make_scaled(MP mp,mp_number*ret,mp_number p_orig,mp_number q_orig){
mpfi_div(ret->data.num,p_orig.data.num,q_orig.data.num,ROUNDING);
mp_check_mpfi_t(mp,ret->data.num);
}

/*:29*//*38:*/
#line 914 "mpmathinterval_new.w"
...
invalid= mpfi_set_str(result,buf,10,ROUNDING);
...
NULL};
mp_snprintf(msg,256,"Required precision is too high (%d vs. numberprecision = %f, required precision=%d bits vs internal precision=%f bits)",(unsigned int)lp,mpfi_get_d(internal_value(mp_number_precision).data.num,ROUNDING),(int)lpbit,precision_bits);
;
...
/*:39*//*40:*/
#line 1005 "mpmathinterval_new.w"
...
/*:40*//*42:*/
#line 1058 "mpmathinterval_new.w"
...
mpfi_set_si(i16,16,ROUNDING);
mpfi_set_si(fone,fraction_one,ROUNDING);
mpfi_set_si(fhalf,fraction_half,ROUNDING);
mpfi_set_si(ftwo,fraction_two,ROUNDING);
mpfi_set_si(sqrtfive,5,ROUNDING);
mpfi_sqrt(sqrtfive,sqrtfive,ROUNDING);
mpfi_div(arg1,sf.data.num,i16,ROUNDING);
mpfi_sub(arg1,st.data.num,arg1,ROUNDING);
mpfi_div(arg2,st.data.num,i16,ROUNDING);
mpfi_sub(arg2,sf.data.num,arg2,ROUNDING);
mp_interval_take_fraction(mp,acc,arg1,arg2);

mpfi_set(arg1,acc,ROUNDING);
mpfi_sub(arg2,ct.data.num,cf.data.num,ROUNDING);
mp_interval_take_fraction(mp,acc,arg1,arg2);

mpfi_sqrt(arg1,two_mpfi_t,ROUNDING);
mpfi_mul(arg1,arg1,fone,ROUNDING);
mp_interval_take_fraction(mp,r1,acc,arg1);
mpfi_add(num,ftwo,r1,ROUNDING);

mpfi_sub(arg1,sqrtfive,one,ROUNDING);
mpfi_mul(arg1,arg1,fhalf,ROUNDING);
mpfi_mul(arg1,arg1,three_mpfi_t,ROUNDING);

mpfi_sub(arg2,three_mpfi_t,sqrtfive,ROUNDING);
mpfi_mul(arg2,arg2,fhalf,ROUNDING);
mpfi_mul(arg2,arg2,three_mpfi_t,ROUNDING);
mp_interval_take_fraction(mp,r1,ct.data.num,arg1);
mp_interval_take_fraction(mp,r2,cf.data.num,arg2);

mpfi_set_si(denom,fraction_three,ROUNDING);
mpfi_add(denom,denom,r1,ROUNDING);
mpfi_add(denom,denom,r2,ROUNDING);

if(!mpfi_equal_p(t.data.num,one)){
mpfi_div(num,num,t.data.num,ROUNDING);
}
mpfi_set(r2,num,ROUNDING);
mpfi_div(r2,r2,four_mpfi_t,ROUNDING);
if(mpfi_less_p(denom,r2)){
mpfi_set_si(ret->data.num,fraction_four,ROUNDING);
}else{
...
/*:42*//*43:*/
#line 1121 "mpmathinterval_new.w"
...
mpfi_set(a,(mpfi_ptr)a_orig.data.num,ROUNDING);
mpfi_set(b,(mpfi_ptr)b_orig.data.num,ROUNDING);
mpfi_set(c,(mpfi_ptr)c_orig.data.num,ROUNDING);
mpfi_set(d,(mpfi_ptr)d_orig.data.num,ROUNDING);

mpfi_mul(q,a,b,ROUNDING);
mpfi_mul(r,c,d,ROUNDING);
cmp= mpfi_cmp(q,r);
if(cmp==0){
mpfi_set(ret->data.num,zero,ROUNDING);
goto RETURN;
}
if(cmp> 0){
mpfi_set(ret->data.num,one,ROUNDING);
goto RETURN;
}
if(cmp<0){
mpfi_set(ret->data.num,minusone,ROUNDING);
goto RETURN;
}


/*44:*/
#line 1194 "mpmathinterval_new.w"

if(mpfi_negative_p(a)){
mpfi_neg(a,a,ROUNDING);
mpfi_neg(b,b,ROUNDING);
}
if(mpfi_negative_p(c)){
mpfi_neg(c,c,ROUNDING);
mpfi_neg(d,d,ROUNDING);
}
...
mpfi_set(q,a,ROUNDING);
mpfi_set(a,c,ROUNDING);
mpfi_set(c,q,ROUNDING);
mpfi_neg(q,b,ROUNDING);
mpfi_neg(b,d,ROUNDING);
mpfi_set(d,q,ROUNDING);
...
/*:44*/
#line 1150 "mpmathinterval_new.w"
;
while(1){
mpfi_div(q,a,d,ROUNDING);
mpfi_div(r,c,b,ROUNDING);
cmp= mpfi_cmp(q,r);
if(cmp){
if(cmp> 1){
mpfi_set(ret->data.num,one,ROUNDING);
...
mpfi_remainder(q,a,d,ROUNDING);
mpfi_remainder(r,c,b,ROUNDING);
if(mpfi_zero_p(r)){
if(mpfi_zero_p(q)){
mpfi_set(ret->data.num,zero,ROUNDING);
}else{
...
mpfi_set(a,b,ROUNDING);
mpfi_set(b,q,ROUNDING);
mpfi_set(c,d,ROUNDING);
mpfi_set(d,r,ROUNDING);
}
RETURN:
#if DEBUG
...
/*:43*//*45:*/
#line 1269 "mpmathinterval_new.w"
...
mpfi_set(a,(mpfi_ptr)aa.data.num,ROUNDING);
mpfi_set(b,(mpfi_ptr)bb.data.num,ROUNDING);
mpfi_set(c,(mpfi_ptr)cc.data.num,ROUNDING);
if(mpfi_negative_p(a))
zero_crossing;
if(!mpfi_negative_p(c)){
if(!mpfi_negative_p(b)){
...
d= epsilonf;
mpfi_set(x0,a,ROUNDING);
mpfi_sub(x1,a,b,ROUNDING);
mpfi_sub(x2,b,c,ROUNDING);
do{

mpfi_add(x,x1,x2,ROUNDING);
mpfi_div(x,x,two_mpfi_t,ROUNDING);
mpfi_add_d(x,x,1E-12,ROUNDING);
mpfi_sub(scratch,x1,x0,ROUNDING);
if(mpfi_greater_p(scratch,x0)){
mpfi_set(x2,x,ROUNDING);
mpfi_add(x0,x0,x0,ROUNDING);
d+= d;
}else{
mpfi_add(xx,scratch,x,ROUNDING);
if(mpfi_greater_p(xx,x0)){
mpfi_set(x2,x,ROUNDING);
mpfi_add(x0,x0,x0,ROUNDING);
d+= d;
}else{
mpfi_sub(x0,x0,xx,ROUNDING);
if(!mpfi_greater_p(x,x0)){
mpfi_add(scratch,x,x2,ROUNDING);
if(!mpfi_greater_p(scratch,x0))
no_crossing;
}
mpfi_set(x1,x,ROUNDING);
d= d+d+epsilonf;
}
}
}while(d<fraction_one);
mpfi_set_d(scratch,d,ROUNDING);
mpfi_sub(ret->data.num,scratch,fraction_one_mpfi_t,ROUNDING);
RETURN:
#if DEBUG
...
/*:45*//*47:*/
#line 1349 "mpmathinterval_new.w"
...
/*:47*//*48:*/
#line 1358 "mpmathinterval_new.w"

void mp_number_floor(mp_number*i){
mpfi_rint_floor(i->data.num,i->data.num,MPFR_RNDD);
}

/*:48*//*49:*/
#line 1364 "mpmathinterval_new.w"

void mp_interval_fraction_to_round_scaled(mp_number*x_orig){
x_orig->type= mp_scaled_type;
mpfi_div(x_orig->data.num,x_orig->data.num,fraction_multiplier_mpfi_t,ROUNDING);
}



/*:49*//*51:*/
#line 1378 "mpmathinterval_new.w"

void mp_interval_square_rt(MP mp,mp_number*ret,mp_number x_orig){
if(!mpfi_positive_p((mpfi_ptr)x_orig.data.num)){
/*52:*/
#line 1389 "mpmathinterval_new.w"
...
mpfi_set_zero(ret->data.num,1);
return;
}


/*:52*/
#line 1381 "mpmathinterval_new.w"
;
}else{
mpfi_sqrt(ret->data.num,x_orig.data.num,ROUNDING);
}
mp_check_mpfi_t(mp,ret->data.num);
}
...
/*:51*//*53:*/
#line 1410 "mpmathinterval_new.w"
...
mpfi_set(a,(mpfi_ptr)a_orig.data.num,ROUNDING);
mpfi_set(b,(mpfi_ptr)b_orig.data.num,ROUNDING);
mpfi_mul(asq,a,a,ROUNDING);
mpfi_mul(bsq,b,b,ROUNDING);
mpfi_add(a,asq,bsq,ROUNDING);
mpfi_sqrt(ret->data.num,a,ROUNDING);
mp_check_mpfi_t(mp,ret->data.num);
mpfi_clears(a,b,asq,bsq,(mpfi_ptr)0);
}

/*:53*//*54:*/
#line 1426 "mpmathinterval_new.w"
...
mpfi_set(a,(mpfi_ptr)a_orig.data.num,ROUNDING);
mpfi_set(b,(mpfi_ptr)b_orig.data.num,ROUNDING);
if(!mpfi_greater_p(a,b)){
/*55:*/
#line 1445 "mpmathinterval_new.w"
...
/*:55*/
#line 1433 "mpmathinterval_new.w"
;
}else{
mpfi_mul(asq,a,a,ROUNDING);
mpfi_mul(bsq,b,b,ROUNDING);
mpfi_sub(a,asq,bsq,ROUNDING);
mpfi_sqrt(a,a,ROUNDING);
}
mpfi_set(ret->data.num,a,ROUNDING);
mp_check_mpfi_t(mp,ret->data.num);
}


/*:54*//*56:*/
#line 1468 "mpmathinterval_new.w"

void mp_interval_m_log(MP mp,mp_number*ret,mp_number x_orig){
if(!mpfi_positive_p((mpfi_ptr)x_orig.data.num)){
/*57:*/
#line 1480 "mpmathinterval_new.w"
...
/*:57*/
#line 1471 "mpmathinterval_new.w"
;
}else{
mpfi_log(ret->data.num,x_orig.data.num,ROUNDING);
mp_check_mpfi_t(mp,ret->data.num);
mpfi_mul_si(ret->data.num,ret->data.num,256,ROUNDING);
}
mp_check_mpfi_t(mp,ret->data.num);
}

/*:56*//*58:*/
#line 1499 "mpmathinterval_new.w"

void mp_interval_m_exp(MP mp,mp_number*ret,mp_number x_orig){
mpfi_t temp;
mpfi_init2(temp,precision_bits);
mpfi_div_si(temp,x_orig.data.num,256,ROUNDING);
mpfi_exp(ret->data.num,temp,ROUNDING);
mp_check_mpfi_t(mp,ret->data.num);
mpfi_clear(temp);
}
...
/*:58*//*59:*/
#line 1513 "mpmathinterval_new.w"
...
/*60:*/
#line 1536 "mpmathinterval_new.w"
...
/*:60*/
#line 1516 "mpmathinterval_new.w"
;
}else{
...
mpfi_set_si(oneeighty_angle,180*angle_multiplier,ROUNDING);
mpfi_div(oneeighty_angle,oneeighty_angle,PI_mpfi_t,ROUNDING);
checkZero((mpfi_ptr)y_orig.data.num);
checkZero((mpfi_ptr)x_orig.data.num);
mpfi_atan2(atan2val,y_orig.data.num,x_orig.data.num,ROUNDING);
mpfi_mul(ret->data.num,atan2val,oneeighty_angle,ROUNDING);
...
/*:59*//*62:*/
#line 1554 "mpmathinterval_new.w"
...
mpfi_set_si(one_eighty,180*16,ROUNDING);
mpfi_mul(rad,z_orig.data.num,PI_mpfi_t,ROUNDING);
mpfi_div(rad,rad,one_eighty,ROUNDING);

mpfi_sin(n_sin->data.num,rad,ROUNDING);
mpfi_cos(n_cos->data.num,rad,ROUNDING);

mpfi_mul(n_cos->data.num,n_cos->data.num,fraction_multiplier_mpfi_t,ROUNDING);
mpfi_mul(n_sin->data.num,n_sin->data.num,fraction_multiplier_mpfi_t,ROUNDING);
...
/*:62*//*63:*/
#line 1578 "mpmathinterval_new.w"
...
/*:63*//*64:*/
#line 1653 "mpmathinterval_new.w"
...
if(k<0)
k+= fraction_one;
mpfi_set_si(mp->randoms[(i*21)%55].data.num,j,ROUNDING);
}
mp_new_randoms(mp);
mp_new_randoms(mp);
mp_new_randoms(mp);
...
/*:64*//*65:*/
#line 1678 "mpmathinterval_new.w"

void mp_interval_number_modulo(mp_number*a,mp_number b){
mpfi_remainder(a->data.num,a->data.num,b.data.num,ROUNDING);
}

/*:65*//*66:*/
#line 1685 "mpmathinterval_new.w"
...
flt_op= op/(MM*1.0);
mpfi_set_d((mpfi_ptr)(rop.data.num),flt_op,ROUNDING);
...
/*:66*//*67:*/
#line 1703 "mpmathinterval_new.w"
...
/*:67*//*68:*/
#line 1719 "mpmathinterval_new.w"
...
mpfi_mul(y.data.num,abs_x.data.num,u.data.num,ROUNDING);
free_number(u);
...
/*:68*//*69:*/
#line 1762 "mpmathinterval_new.w"
...
/*:69*//*70:*/
#line 1809 "mpmathinterval_new.w"
...
mpfi_mul(ab,a,b,ROUNDING);
mpfi_mul(cd,c,d,ROUNDING);
