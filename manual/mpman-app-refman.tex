...
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  ...
  assignment is applied just before the input file is read-in.  String
  values have to be put into double quotes.  Double quotes are stripped,
  but no other string processing takes place.  This switch can be
  applied multiple times on the command-line.\newline
...
The \ttindex{dvitomp}\label{Ddvitomp} program converts DVI files into
