...
\item[\cmdin] A MetaPost input file is a text file containing statements
  ...
  \begin{enumerate}
  \item If \cmdin\ doesn't end with \texttt{.mp}\index{mp file?{\tt mp}
      file}\index{files!mp?{\tt mp}}, MetaPost first looks for a file
    \texttt{\cmdin.mp}.  Only if that file doesn't exist, it looks for
    file \cmdin\ literally.  That way, the \texttt{mp}\index{mp
      file?{\tt mp} file}\index{files!mp?{\tt mp}} file extension can be
    omitted when calling MetaPost.
  \item If \cmdin\ already ends with \texttt{.mp}\index{mp file?{\tt mp}
      file}\index{files!mp?{\tt mp}}, no special file name handling
    takes place and MetaPost looks for that file only.
  \end{enumerate}
...
$$
\begin{verbatim}[commandchars=\\\{\}]
dvitomp \opt{\cmdsw} \cmdin \cmdout
\end{verbatim}
\index{dvitomp?\texttt{dvitomp}}\index{command-line!dvitomp?\texttt{dvitomp}}
$$

\begin{description}
\item[\cmdin] This is the DVI file to convert.  This file is, driven by
  Metapost, in general automatically generated by \TeX\ or troff.

\item[\cmdout] This is the output file containing the MetaPost code
  equivalent to \cmdin.  The file name ends with \texttt{.mpx}\index{mpx
    file?{\tt mpx} file}\index{files!mpx?{\tt mpx}}.
