...
\label{refman:mpost}
\index{command-line!\texttt{mpost}}
...
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  ...
  \cmdindex{-{}-dvitomp}
  & Act as the \textit{dvitomp} executable (see
  section~\ref{refman:dvitomp}).\\
\end{longtable}
...
\index{command-line!\texttt{dvitomp}}

The \ttindex{dvitomp} program converts DVI output into low-level
...
contains the equivalent MetaPost code.

When delegating the typesetting of \verb|btex| \verb|...| \verb|etex|
text labels to \TeX\ or \emph{troff}, MetaPost uses the \texttt{dvitomp}
program for the final conversion back into MetaPost code.  Files
generated by \texttt{dvitomp} during \verb|btex| \verb|...| \verb|etex|
processing end with \texttt{.mpx}.

The \texttt{dvitomp} program used to be part of a set of external tools,
called \emph{mpware}\index{mpware tools}, that were used by MetaPost for
processing \verb|btex| \verb|...| \verb|etex| labels.  Since MetaPost
version~1.100, the \verb|btex| \verb|...| \verb|etex| label conversion
is handled internally by the \texttt{mpost} executable.  The
\emph{mpware} tools are therefore obsolete and no longer part of the
MetaPost distribution.  The \texttt{dvitomp} executable is actually a
wrapper, calling \texttt{mpost} as
