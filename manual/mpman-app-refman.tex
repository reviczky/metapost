...
\item[\texttt{\&\cmdmem}] The MetaPost program code in the \cmdmem\
  MetaPost source file (and any other files that it inputs) is
  executed before all other processing starts. The \cmdmem\ file
  should end with an \texttt{end} or \texttt{dump} command.

  If \cmdmem\ nor \cmdindex{-mem} is specified on the command line,
  MetaPost will default to loading the macro file \texttt{mpost}, or
  to whatever the actual name of the MetaPost executable is. The 
  \cmdindex{-ini} switch can be used to suppress this behavior.
