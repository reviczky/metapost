...
\NC mp_stream \NC ship_out    \NC holds the exported EPS or SVG string \NC \NR
...
Fills in the coordinate of knot \type{p}. \type{fabs(x)} and \type{fabs(y)} should be less than 4096.0.
...
of the path, and \type{q} is the first knot. 
Because all knot lists are cyclic, this is needed to end the path 
properly (or use \type{mp_close_path_cycle()}).
Return 1 on success, 0 on failure.
...
of the path, and \type{q} is the first knot. 
This is needed to mimic metapost's 'cycle'.
return 1 on success, 0 on failure.
...
Sets explicit controls for a knot pair.
\type{fabs(x1)} and \type{fabs(y1)} should be less than 4096.0
\type{fabs(x2)} and \type{fabs(y2)} should be less than 4096.0
return 1 on success, 0 on failure.
