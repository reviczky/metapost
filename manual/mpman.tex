...
defined by the font encoding\index{encoding}\index{font!encoding}.  A
glyph\index{font!glyph} is the visual representation of a character in a
font.  In general, the same character is represented by different glyphs
in different fonts.  Figure~\ref{glyphsample} shows some glyphs for the
...
To save the strokes of a glyph in a picture, the
...
has more on font names).

MetaPost then looks-up the font name in the font map to determine the
encoding and to find the font's
PostScript\index{PostScript!fonts}\index{font!PostScript} source file
(see section~\ref{Sfontmapfile}).  The glyphs of a PostScript font are
composed of two types of contours: Clockwise oriented contours add to
the shape of a glyph and are drawn in black ink.  Counter-clockwise
oriented contours erase parts of other contours, i.e., make them
transparent again.  The \verb|glyph| operator returns a picture
consisting of these contour lines explicitly colored black and white in
the greyscale color model according to the rules laid out above.  The
coloring is necessary for the picture to resembles the corresponding
glyph \emph{visually}\footnote{Plain contours already carry enough
  information to completely reconstruct a glyph, since the orientation
  of a contour can be computed from its cardinal and control points.
  MetaPost has a
...
in the original glyph.  Finally, the picture is scaled by a factor of
$\frac{1\,\mbox{\scriptsize pt}}{\mbox{\scriptsize font design unit}}$.

The \verb|glyph|\index{glyph?\texttt{glyph}} operator accepts another,
more low-level, syntax:
