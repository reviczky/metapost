...
PostScript programs or SVG\index{SVG} graphics instead of bitmaps.  
...
fig.1}\index{files!output}, by default.  In other words, the numeric
argument to the \verb|beginfig| macro determines the name of the
corresponding output file.  See section~\ref{outputfilenames} for more
...
The default output of MetaPost is a variant of PostScript, called Encapsulated
...
Starting with version~1.110, MetaPost is also capable of producing 
SVG output. SVG files can be previewed with certain web browsers,
for example Firefox~3, and it can be imported by various interactive
graphics editing programs, for example Inkscape. This method is not
suitable for inclusion of MetaPost graphics in \TeX\ documents.
...
\ttt{.2}, etc.}  The MetaPost primitive \verb+outputtemplate+ can be
used to set the file extension of MetaPost output to \ttt{mps} (see
section~\ref{outputfilenames}). Hence, no \verb+\DeclareGraphicsRule+
...
If you're using the \ttt{outputtemplate} method the \ttt{mps} file
...
There is an internal string variable called \ttt{outputtemplate}%
\index{outputtemplate?\texttt{outputtemplate}}\label{Doutputtemplate},
that you can assign template strings to:

\begin{center}\begin{tabular}{l}
\verb|outputtemplate := "%j-%3c.mps";|\\
\verb|beginfig(1);|\\
\verb|   draw p;|\\
\verb|endfig;|
...
The \ttt{outputtemplate} primitive can also be helpful for naming
...
MetaPost version 1.000 used a different primitive with the name
\ttt{filenametemplate}. This primitive is still provided, but its 
use is deprecated.

\subsection{Selecting the output format}

By default, MetaPost generates PostScript language output. If you
prefer to have SVG output, you can set the internal variable
\ttindex{outputformat}\label{Doutputformat} to \ttt{svg}:

\begin{center}\begin{tabular}{l}
\verb|outputformat := "svg";|\\
\verb|outputtemplate := "%j-%3c.svg";|\\
\verb|beginfig(1);|\\
\verb|   draw p;|\\
\verb|endfig;|
\end{tabular}\end{center}

For SVG, it makes sense to set the file extension to \ttt{svg}. At
the moment, this has to be done explicitly, but in one of the upcoming
development versions it will become possible to use a template escape
instead.
...
\tt     outputformat&    string& \pageref{Doutputformat}&
        Output backend to be used\\\hline
\tt     outputtemplate&    string& \pageref{Doutputtemplate}&
        Output filename template\\\hline
...
The MetaPost executable understands a number of command-line switches
that control the run-time behavior.  Here's a summary:
...
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  ...
  \ttindex{-{}-kpathsea-debug}=\textit{NUMBER}
  & Set debugging flags for path searching.\\
  \ttindex{-{}-mem}=\textit{STRING}
  & ???\\
  \ttindex{-{}-jobname}=\textit{STRING}
  & Set the name of the job (affects output file names).\\
  \ttindex{-{}-progname}=\textit{STRING}
  & Pretend to be the \texttt{\textit{STRING}} executable.\\
  \ttindex{-{}-troff}, \ttindex{-{}-T}
  & Output \textit{troff} compatible PostScript files.\\
  \ttindex{-{}-recorder}
  & ???\\
  \ttindex{-{}-tex}=\textit{TEXPROGRAM}
  & Load format \texttt{\textit{TEXPROGRAM}} for rendering \TeX\
  material.\\
  \ttindex{-{}-interaction}=\textit{STRING}
  & Set interaction mode to one of \texttt{\itshape batchmode,
    nonstopmode, scrollmode, errorstopmode}.\\
  \ttindex{-{}-no-kpathsea}
  & ???\\
  \ttindex{-{}-file-line-error}
  & ???\\
  \ttindex{-{}-no-file-line-error}
  & ???\\
  \ttindex{-{}-help}
  & Show help on command-line switches.\\
  \ttindex{-{}-version}
  & Print version information and exit.\\
  \ttindex{-{}-halt-on-error}
...
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  \ttindex{-{}-8bit}
  & \\
  \ttindex{-{}-parse-first-line}
  & \\
\end{longtable}

The following command-line switches are ignored, but trigger a warning:
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  \ttindex{-{}-translate-file}=\textit{STRING}
  & \\
  \ttindex{-{}-output-directory}=\textit{STRING}
  & \\
  \ttindex{-{}-no-parse-first-line}
  & \\
\end{longtable}
