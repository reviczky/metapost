...
MetaPost's graphic design workflow.  This section also contains some
technical details about MetaPost that are useful to know.  If you're not
interested in these basics right now you can savely skip those parts and
only refer to them later.

Technically, the MetaPost compiler is a library written in the CWeb
programming language, called
\emph{mplib}\index{mplib?\emph{mplib}}.\footnote{C
  API\index{mplib?\emph{mplib}!C API} of and Lua
  bindings\index{mplib?\emph{mplib}!Lua bindings} to \emph{mplib} are
  described in
  \verb|manual/mplibapi.pdf|\index{mplib?\emph{mplib}!\texttt{mplibapi.pdf}}
  as part of the MetaPost distribution.}  In this manual, we'll assume a
stand-alone command-line executable of the MetaPost compiler---a small
wrapper program around \emph{mplib}---is used.  The library can as well
be embedded into third-party applications.  Appendix~\ref{mpimport}
briefly discusses using the \emph{mplib} library built-into Lua\TeX.
For more information, please refer to the documentation of the embedding
application.

The basic MetaPost workflow is depicted in figure~\ref{fig:mpworkflow}.
Being a graphics description language, creating MetaPost graphics
follows the \emph{edit-compile-debug} paradigm known from other
...
Appendix~\ref{mpimport} deals with the import of MetaPost graphics into
...
\paragraph{Previewing MetaPost graphics}
\label{previewing}
\index{previewing}
...
viewer.\footnote{There are alternatives, though.  mpstoeps\index{mpstoeps} is
a Perl script that automates the process outlined above.
mptopdf\index{mptopdf} is another tools that converts MetaPost files to
PDF.}
...
reliably be previewed, whether they contain text or not.  The new
behaviour can be triggered by setting MetaPost's internal variable
\ttt{prologues}\index{prologues?\texttt{prologues}} to~3.  See
section~\ref{Dbtex} for more information on \ttt{prologues}.

Starting with version~1.200, MetaPost is also capable of producing SVG
output.  SVG files can be previewed with certain web browsers, for
example Firefox~3\index{Firefox} or Konqueror~4.2\index{Konqueror}.
