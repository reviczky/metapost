...
(SVG\index{SVG}) that follow version~1.1 of the SVG specification
(\url{http://www.w3.org/TR/SVG11/}).  Both are vector formats that can
be scaled without quality degradation.  It seems appropriate to point
out, that despite scalability, vector graphics, too, are best created at
nearly target size, since the visual character of a graphic might change
at different scales.  This is, because certain properties might not
scale proportionally, e.g., line width or arrow size.  On the other
hand, while outline fonts do scale proportionally, this is not always
desired, e.g., when optical sizes are available, as for the Computer
Modern font family.

By default, MetaPost outputs PostScript files.  The output format can be
changed to SVG by assigning the value \verb|"svg"| to the internal
string variable \ttindex{outputformat}\label{Doutputformat}:
...
As discussed in section~\ref{processing}, by default, every
...
\subsection{Handling PostScript output}
\label{teximport}
...
How PostScript figures can be integrated into documents prepared with
...
\subsection{Handling SVG output}
\label{svgimport}

SVG graphics can be easily embedded into HTML\index{HTML} documents:
...
SVG graphics can also be imported by various interactive graphics
