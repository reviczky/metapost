...
$$ {\tt mpost}\ \descr{switch}\ \descr{file} $$
...
All switches are prefixed by one or two dashes.  Both forms have the
same meaning.  An exemplary call to MetaPost that compiles file
...
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  ...
  \textit{kpathsea}.)\\
  \ttindex{-{}-tex}=$\descr{texprogram}$
  & Load format $\descr{texprogram}$ for rendering \TeX\ material.\\
  \ttindex{-{}-interaction}=$\descr{string}$
  ...
  \ttindex{-{}-dvitomp}
  & Act as the \textit{dvitomp} executable.  (This implies a different
  command-line.)\\
\end{longtable}
...
\begin{longtable}{>{\ttfamily}p{.3\linewidth}>{\raggedright\arraybackslash}p{.6\linewidth}}
  \ttindex{-{}-translate-file}=$\descr{string}$
  & \\
  \ttindex{-{}-output-directory}=$\descr{string}$
  & \\
