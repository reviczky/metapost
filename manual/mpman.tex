...
programming language, called \emph{mplib}\index{mplib?\emph{mplib}}.  A
small wrapper program turns this library into a stand-alone command-line
application.  Alternatively, \emph{mplib} can be embedded by third-party
applications, like e.g. Lua\TeX.  The communication between MetaPost and
third-party applications depends on how MetaPost is invoked.  In this
manual, we'll assume a stand-alone executable of the MetaPost compiler
is used.  Appendix~\ref{mpimport} briefly discusses using the
\emph{mplib} library built-into Lua\TeX.  For more information, please
refer to the documentation of the embedding application.
