...
It seems advisable to point out, that despite the fact, vector graphics
can be scaled without quality degradation, they, too, are best created
at nearly target size.  Otherwise the visual character of a drawing
might change at different scales.  This is, because certain properties
of a drawing do not scale proportionally, e.g., line width or arrow
size.  On the other hand, while outline fonts do scale proportionally,
this is not always desired, e.g., when a font is available in optical
sizes, like the Computer Modern font family.
...
the value \verb|"svg"| to the internal string variable
...
\verb|"SVG"| enables PostScript output, too.
...
\begin{table}
  ...
  \begin{tabular}{|>{\ttfamily}l|l|}
    ...
    \%\%   & percent sign \\
    \%j    & current jobname\\
    \%\d c & charcode value (\verb|beginfig| argument)\\
    \%\d y & current year\\
    \%\d m & month (numeric)\\
...
section~\ref{Doutputtemplate}).  In the \LaTeX\ document include the
...
environment variable \ttindex{MAKEMPX} is set, this external program 
...
subpath runs backwards along~$p$.  For an in-depth discussion of the
\verb|subpath| operator the interested reader should refer
to~\cite[p.~133]{kn:c}.
...
The above mentioned operators all operate on points that lay \emph{on}
paths.  There's one important path property that involves point that in
general do not lay on a path: the control points of a path's B\'ezier
segments.  MetaPost provides two operators for evaluating the control
points of a path:
\begin{eqnarray*}
  && \verb|precontrol|\, \descr{numeric expression}\, \verb|of|\,
  \descr{path}\index{precontrol?\texttt{precontrol}}\label{Dprecontrol}\\
\end{eqnarray*}
that work as follows: Consider a \emph{non-cyclic} path $p = z_0\ .\,.\
z_1\ .\,.\ \descr{etc.}\ .\,.\ z_i\ .\,.\ z_{i+1}\ .\,.\ \descr{etc.}\
.\,.\ z_{n-1}\ .\,.\ z_n$ of length~$n$.  The following rules apply:

\begin{eqnarray}
  \label{eq:cptnoncyc}
  \hbox{\verb|postcontrol t of p|} & = & \left\{
    \begin{array}{ll}
      \hbox{first control point in \verb|subpath (t,n) of p|} & \hbox{,
        if $t<n$,}\\
      z_n & \hbox{, if $t\ge n$}.
    \end{array}
    \right.\\
  \hbox{\verb|precontrol t of p|} & = & \left\{
    \begin{array}{ll}
      \hbox{last control point in \verb|subpath (0,t) of p|} & \hbox{,
        if $t>0$,}\\
      z_0 & \hbox{, if $t\le 0$}.
    \end{array}
    \right.\\
...
For cyclic paths $q = p\ .\,.\ \hbox{cycle}$ 

cycletime values $t<n$,
\verb|postcontrol t of p| is the first control point in
\verb|subpath (t,n) of p|.  For $t\ge n$, $z_n$ is returned.



For cardinal time values~$i$ and $i+1$, the expressions
\verb|postcontrol i of p| and \verb|precontrol i+1 of p| return the two
control points, that determine the shape of the $i$th cubic B\'ezier
segment $z_i\ .\,.\ z_{i+1}$ of $p$.  For fractional time values the
following



For the end points of a path the following rules apply:

\begin{eqnarray}
  \hbox{\verb|precontrol 0 of p|} & = & \left\{
    \begin{array}{ll}
      z_0 & \hbox{, if $p$ is non-cyclic}\\
      \hbox{\verb|precontrol|}\ z_n & \hbox{, if $p$ is cyclic}\\
    \end{array}
  \right.\\
  \hbox{\verb|postcontrol n of p|} & = & \left\{
    \begin{array}{ll}
      z_n & \hbox{, if $p$ is non-cyclic}\\
      \hbox{\verb|postcontrol|}\ z_0 & \hbox{, if $p$ is cyclic}\\
    \end{array}
  \right.
\end{eqnarray}

The operators \verb|postcontrol| and \verb|precontrol| are defined for
fractional time values, too.  A general definition of 
