...
There are two transformations applied to the
\verb|%c|\index{\%c?\texttt{\%c}} pattern.  For positive arguments to
\verb|beginfig|, \verb|%c| is the
charcode\index{charcode?\texttt{charcode}} value rounded to the nearest
integer.  For negative arguments, \verb|%c| evaluates to the string
\verb|ps| for backwards compatibility.
