...
CharString name is a unique text label for a glyph in a font (a font
encoding actually maps CharStrings to slots).  This second syntax can be
used to address glyphs without having to think about font encodings.
The second argument to the \verb|glyph| operator is a string containing
a font name (section~\ref{text} has more on font names).
...
The contours in the picture are no raw copies of the contours found in
the font sources, but the \verb|glyph| operator applies some
transformations.  First, the direction of all contours is reversed, so
that contours filled black become counter-clockwise oriented
(mathematically positive) and contours filled white become clockwise
oriented (mathematically negative).  Second, the \verb|closepath|
operator\index{PostScript!\texttt{closepath} operator} of the PostScript
language closes paths with a straight line, like
\verb|--cycle|\index{cycle?\texttt{cycle}} in MetaPost.  Therefore, for
paths that end with a curved path segment, the last point matches the
starting point of the path, so that the closing line segment has a
...
$$
MetaPost's \verb|glyph| operator transforms paths into the latter
representation, whenever it finds a contour with identical start and end
points.  Finally, the picture returned by the \verb|glyph| operator is
scaled such that one font design unit\index{font!design unit} equals one
PostScript point\index{point!PostScript}\index{PostScript!point} (bp).
Usually, a font design unit is a thousandth part of the font design
size\index{font!design size}.  So, the picture returned by the glyph
operator will typically have a height of around 1000\,bp.
