% $Id: mpman.tex 2017 2014-05-10 21:14:45Z stephanhennig $
...
\svnInfo $Id: mpman.tex 2017 2014-05-10 21:14:45Z stephanhennig $
    ...
    \raisebox{0pt}[0pt][0pt]{%
      \begin{minipage}{\linewidth}
        \emph{The manual currently doesn't cover all recent features of
          MetaPost, such as the support for calculations in double,
          decimal, and binary number systems.  We'll try to keep up.
          For more information, please refer to the
          \href{https://foundry.supelec.fr/scm/viewvc.php/trunk/CHANGES?view=markup&root=metapost}{change
...
MetaPost utilizes the Cairo\index{Cairo} graphic library.  Bitmap
...
be found in table~\ref{tab:outputformatoptions}.  By default, variable
\texttt{outputformatoptions} is empty.  An assignment that would match
the compiled-in default setup would look like
...
white.  All color channels have a bit-depth of~8.  Note, the PNG format
has no support for the CMYK color space.  Here's what color conversions
are applied during bitmap conversion.  Whatever input and output color
spaces are used, all colors are initially converted to RGB color space.
A gray scale color with intensity $i$ is converted to an RGB color with
all components set to~$i$.  RGB colors are left unchanged.  A CMYK color
with components $(c,m,y,k)$ is converted to an RGB color with components
$(r,g,b)$ using equations
\begin{eqnarray}
  r &=& \max(1 - k - c, 0)\\
  g &=& \max(1 - k - m, 0)\\
  b &=& \max(1 - k - y, 0)
\end{eqnarray}
If requested by the output format (\texttt{gray}, \texttt{graya}), such
a color is further converted to gray scale with an intensity~$j$ using
equation
\begin{eqnarray}
...
are compatible, \emph{i.e.}, both are RGB or gray scale or an RGB format
PNG file contains only gray scale colors.  Users that care about color
...
Section~\ref{Dforwithin} and Appendix~\ref{Dxefig}).  The built-in
conversions are provided just as a convenience.

Resolution of PNG output is always 72~dpi\index{dpi}\index{units!dpi}
(dots per inch).  The two internal variables
\texttt{hppp}\index{hppp?\texttt{hppp}}\label{Dhppp} and
\texttt{vppp}\index{vppp?\texttt{vppp}}\label{Dvppp} are used by the PNG
backend to decide on the scale of the generated bitmap.  Default value
for both variables is $1.0$, \emph{i.e.}, one pixel per MetaPost point.
These two variables have already been present in MetaFont, which outputs
bitmaps only, and have been revived when MetaPost acquired the bitmap
backend in version 1.800.
