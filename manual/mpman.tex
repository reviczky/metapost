...
files\index{files!input} normally have names ending \texttt{.mp} but
...
$\descr{filename}{\tt.log}$.  This includes error messages and any
MetaPost commands entered interactively.\footnote{A {\tt
...
The file extension \texttt{.mps} is conventionally chosen for MetaPost's
PostScript output (see section~\ref{teximport}).  For SVG output one
would want to use \texttt{.svg} instead.
...
with file extension \texttt{.mps} are recognized as purified EPS and can
be converted to PDF\index{PDF} on-the-fly.  The recommended procedure
for including MetaPost graphics into \LaTeX\ documents compiled with the
pdf\TeX\ engine should be obvious: change MetaPost's output file naming
scheme to write files ending \texttt{.mps} via
...
$$
where the first argument is a slot number between 0 and~255 and the
second argument is a string containing a font name (section~\ref{text}
has more on font names).  Fractional slot numbers are rounded to the
...
glyph \emph{visually}\footnote{Plain contours already carry enough
  information to completely reconstruct a glyph, the orientation of a
  contour can be computed from its cardinal and control points.
  MetaPost has a
  \texttt{turningnumber}\index{turningnumber?\texttt{turningnumber}}\label{Dturningnumber}
  primitive to do that.}, since MetaPost doesn't implement PostScript's
even-odd fill rule\index{even-odd fill rule}\index{fill
  rule!even-odd}\index{PostScript!fill rule}.  As a side effect, the
interiors of the erasing\index{erasing} contours are an opaque white in
the returned picture, while they were transparent in the original glyph.
One can think of erasing contours to be unfilled (see
p.~\pageref{Dunfill}).  Finally, the picture is scaled such that one
font design unit\index{font!design unit} equals one PostScript
point\index{point!PostScript}\index{PostScript!point} (bp).  Typically,
a font design unit is a thousandth part of the font design
size\index{font!design size}.  So, the picture returned by the glyph
operator will often have a height of around 1000\,bp.  For instance, the
following code saves the contours of the lower case letter~``g'' in the
Computer Modern Roman\index{Computer Modern Roman} font in a picture
variable.  The glyph~``g'' is bound to slot~103 in the OT1
encoding\index{OT1 encoding}\index{encoding!OT1}:
...
or \texttt{.pfb}\index{pfb file?{\tt pfb} file}\index{files!pfb?{\tt
    pfb}} file or the encoding cannot be found or the slot number is not
covered by the encoding.  Note, while MetaPost delegates the actual font
handling to a rendering application for \verb|infont| and \verb|btex|
\ldots\ \verb|etex| blocks, the \verb|glyph| operator directly operates
on font ressources.  For that reason, a font map entry is mandatory for
the font in question, given either by \verb|fontmapline| or
\verb|fontmapfile| (see section~\ref{Dfontmapline}).

Another, more low-level, syntax is possible for the
\verb|glyph|\index{glyph?\texttt{glyph}} operator:
$$ \verb|glyph|\, \descr{string expression}\, \verb|of|\, \descr{string
  expression}
$$
Here, the first argument is not a slot number, but a string containing a
CharString name\index{CharString name} in the PostScript font's source
file.  A CharString name is a unique text label for a glyph in a font.
A font encoding actually maps CharStrings to slots.  This second syntax
can be used to address glyphs without having to think about font
encodings.  If the CharString name cannot be found, the \verb|glyph|
operator returns an empty picture.  In Figure~\ref{fig56}, the contours
of the upper case letter~``\v{D}'' in the Latin Modern Roman\index{Latin
  Modern Roman} font are saved in a picture variable.  The glyph is
identified by its CharString name ``Dcaron''\index{Dcaron}.  The code
then iterates over all contours and draws them together with their
cardinal (black) and control points (red).  As it turns out, many of the
control points coincide with cardinal points in this glyph.
...
For font experts, here's another implementation detail: PostScript's
\verb|closepath| operator\index{PostScript!\texttt{closepath} operator}
closes paths with a straight line, like
\verb|--cycle|\index{cycle?\texttt{cycle}} in MetaPost.  Therefore, for
paths that end with a curved path segment, the last point has to match
the starting point of the path, so that the closing line segment has a
length of zero.  The MetaPost representation of such a path would be:
$$
z_1\verb|..|\mbox{\itshape controls}\verb|..|z_2\verb|..| \cdots
\verb|..|z_n\verb|..|\mbox{\itshape controls}\verb|..|z_1\verb|--cycle|
$$
Since MetaPost can close paths with a curved path segment via
\verb|..cycle|, a more natural representation of this path would be:
$$
z_1\verb|..|\mbox{\itshape controls}\verb|..|z_2\verb|..| \cdots
\verb|..|z_n\verb|..|\mbox{\itshape controls}\verb|..cycle|
$$
MetaPost's glyph operator transforms paths into the latter
representation, whenever it finds a contour with identical start and end
points.
