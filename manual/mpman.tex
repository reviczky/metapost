...
\verb|%{...}|), there can be an optional number (0--9) after \verb|%|
that determines the number of digits in the resulting string expression,
like \verb|%2m|.  The string is then padded with zeros from left.  When
the \verb|%c|\index{\%c?\texttt{\%c}} escape sequence is evaluated, two
transformations are applied.  For positive arguments to \verb|beginfig|,
\verb|%c| is the charcode\index{charcode?\texttt{charcode}} value
rounded to the nearest integer.  For negative arguments, \verb|%c|
evaluates to the string \verb|ps| for backwards compatibility.
