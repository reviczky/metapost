...
glyph and are drawn in black ink.  Counter-clockwise oriented contours
erase parts of other contours, i.e., make them transparent again.  To
save the strokes of a glyph in a picture, the
...
even-odd fill rule\index{even-odd fill rule}\index{fill
  rule!even-odd}\index{PostScript!fill rule}.  As a side effect, erasing
contours are opaque in the returned picture, while they were transparent
in the original glyph.  Finally, the picture is scaled such that one
point equals one font design unit.  For instance, the following code
saves the strokes of the glyph at slot~103 of the OT1 encoded\index{OT1
  encoding}\index{encoding!OT1} Computer Modern Roman\index{Computer
  Modern Roman} font, i.e., the lower case letter~``g,'' in a picture
variable:
...
covered by the encoding.  Note, while \verb|btex| \ldots\ \verb|etex|
blocks and \verb|infont| delegate the actual font handling to third
party programs---a rendering application---, the \verb|glyph| operator
directly operates on font ressources.  For that reason, a font map entry
is required for the font in question, given either by \verb|fontmapline|
or \verb|fontmapfile| (see section~\ref{Sfontmapfile}).
...
saves the strokes of the glyph with CharString name ``Rcaron'' of the
Cork encoded\index{Cork encoding}\index{T1
  encoding}\index{encoding!Cork}\index{encoding!T1} Latin Modern
Roman\index{Latin Modern Roman} font, i.e., the upper case
letter~``\v{R},'' in a picture variable:
