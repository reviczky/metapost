...
preamble declaration for enhanced text rendering.  The remainder of this
section briefly introduces three assignments to internal variables, each
one useful by itself, that can often be found in MetaPost input files:
$$
\begin{verbatim}[commandchars=\\\{\}]
...
when processing MetaPost files through the mptopdf\index{mptopdf} utility
...
\begin{table}
  ...
  \begin{tabular}{|>{\ttfamily}l|l|>{\ttfamily}l|}
    \hline
    \multicolumn1{|c|}{Escape sequence} & \multicolumn1{c|}{Meaning} & \multicolumn1{c|}{Equivalent}\\\hline
...
In escape sequences referring to numeric internals (including
\verb|%{...}|), there can be an optional number (0--99) after \verb|%|
that determines the minimum number of digits in the resulting string
expression, like \verb|%2m|.  The actual string length may exceed the
requested value, if the decimal representation of the internal variable
needs more digits.  If it needs fewer digits, the string is padded to
the requested length with zeros from left.

When the \verb|%c|\index{\%c?\texttt{\%c}} escape sequence is evaluated,
two transformations are applied.  First, the
charcode\index{charcode?\texttt{charcode}} value is rounded to the
nearest integer.  Second, if the result is negative, \verb|%c| evaluates
to the string \verb|ps| for backwards compatibility.  The latter
transformation can be bypassed by using \verb|%{charcode}| instead of
\verb|%c|.
...
\emph{As of October 2010, \texttt{dvips}\index{dvips?\texttt{dvips}}
  v5.99 doesn't handle MetaPost output gracefully, if variable
  \texttt{prologues}\index{prologues?\texttt{prologues}} is set to a
  value larger than~0.  It is therefore \textsc{mandatory} to reset
  \texttt{prologues} to~0 before including MetaPost graphics into \TeX\
  documents that are post-processed by \texttt{dvips}, currently.}

\paragraph{Proof Sheets}
...
MetaPost input file in a proof\index{proof sheet} sheet, a multi-page
document, that can be previewed and navigated in a single instance of
the viewer application.

The MetaPost distribution contains two scripts, \texttt{mproof.tex} and
\texttt{mpsproof.tex}, that help with the latter approach.  The scripts
are in (plain) \TeX, no MetaPost at all.  To write a proof sheet for
MetaPost output, call \ttindex{mproof.tex} as
$$
\begin{verbatim}[commandchars=\\\{\}]
...
sheets MetaPost graphics are already embedded.  \emph{But notice the
  final remark on text labels and \texttt{dvips} in the preceeding
  section.}
...
The \ttindex{mpsproof.tex} script is similar to \texttt{mproof.tex}, but
more powerful.  While \texttt{mproof.tex} only runs with \TeX\ and
requires an additional DVI output driver to generate PostScript files,
\texttt{mpsproof.tex} can as well be run through pdf\TeX\ to directly
generate PDF files.

Syntax is the same as for \texttt{mproof.tex}, but there are two
additional options.  With the
\verb|\noheaders|\index{mpsproof.tex!noheaders?\texttt{\\noheaders}}
option, file names, date stamps, and page numbers are omitted from the
proof sheet.  Use it like
$$
\begin{verbatim}[commandchars=\|\{\}]
...
The
\verb|\encapsulate|\index{mpsproof.tex!encapsulate?\texttt{\\encapsulate}}
option can be used to generate an output file that has exactly the same
page size as a figure's bounding box.  With this option only one figure
can be processed, e.g.,
$$
\begin{verbatim}
pdftex mpsproof \encapsulate fig.1
\end{verbatim}
$$
